...
public interface ParticipationRepository extends JpaRepository<Participation, Integer> {
    ...
    List<Participation> findAllByUserIdAndEventId(Integer userId, Integer eventId);
    ...
    @Query("select p from Participation p where p.event in ?1 and p.status = 'CONFIRMED'")
    List<Participation> findAllRequestsByEventIn(List<Event> events);
}
